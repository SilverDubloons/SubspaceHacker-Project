...
public class MovingButton : MonoBehaviour, IPointerDownHandler, IPointerEnterHandler, IPointerExitHandler, IPointerUpHandler
{
	...
	public Label label;
	public int clicksInARow;
	public float timeOfLastClick;
	public const float maxTimeBetweenConsecutiveClicks = 0.4f;
	...
	public void OnPointerUp(PointerEventData pointerEventData)
    {
		if(!disabled)
		{
			...
			{
				// Debug.Log($"Time.time = {Time.time} timeOfLastClick = {timeOfLastClick} Time.time - timeOfLastClick = {Time.time - timeOfLastClick} maxTimeBetweenConsecutiveClicks = {maxTimeBetweenConsecutiveClicks} clicksInARow = {clicksInARow}");
				if(Time.time - timeOfLastClick > maxTimeBetweenConsecutiveClicks)
				{
					clicksInARow = 0;
				}
				timeOfLastClick = Time.time;
				clicksInARow++;
				if(clicksInARow >= 2) // button was double clicked
				{
